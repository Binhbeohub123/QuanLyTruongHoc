...
public class QuanLyHocSinh_View extends javax.swing.JPanel {
    ...
    // 3. Gọi Controller để thêm mới hoặc cập nhật
    if (isEditing) {
    ...
    // 4. Refresh bảng và làm mới form
    loadTable();   // hoặc loadTableData()
    lamMoi();      // reset form về trạng thái ban đầu
}
private void lamMoi() {
    // Ẩn date picker nếu đang hiện
    if (currentDateChooser != null) {
        jPanel5.remove(currentDateChooser);
        currentDateChooser = null;
        isDatePickerVisible = false;
    }
    // Chuyển tiêu đề tab sang “Thêm Học Sinh Mới”
    jLabel21.setText("Thêm Học Sinh Mới");

    // Thiết lập lại field ngày
    setupDateField();

    // Sinh mã HS mới
    String newMa = controller.sinhMaHocSinhMoi();
    jTextField14.setText(newMa);
    jTextField14.setEditable(false);
    ...
    jComboBox2.setSelectedIndex(0); // Mã lớp

    // Xóa ảnh đã chọn
    selectedImageBase64 = null;      // biến giữ ảnh khi chọn
    jLabel22.setIcon(null);
    jLabel23.setText("Chưa có ảnh");

    // Ẩn nút Lưu (nếu bạn chỉ hiện Save khi đang edit)
    jButton9.setVisible(false);

    // Reset date picker textfield
    setSelectedDate(null);
    
    // Quay về tab Danh sách nếu muốn
